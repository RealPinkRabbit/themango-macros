...
// ── [A] "카테고리를 변경해주세요"(무효표시) 코드 학습·차단 ──
// 마커는 저장값 옵션 텍스트에만 나타나므로(선택 시점엔 안 보임), 페이지 로드 시 그 코드를 학습해
// 이후 선택에서 배제한다. 이미 저장된 필터는 로드 시 마커가 보이므로 같은 실행에서 바로 차단됨.
function getAvoid(){ try{ return JSON.parse(localStorage.getItem('tmg_catmap_avoid'))||{}; }catch(e){ return {}; } }
function saveAvoid(a){ try{ localStorage.setItem('tmg_catmap_avoid', JSON.stringify(a)); }catch(e){} }
var AVOID=getAvoid();
function isAvoided(market, val){ var l=AVOID[market]; return !!(l && val && l.indexOf(val)>=0); }
function learnInvalid(){
  var changed=false;
  MARKETS.forEach(function(m){
    var s=document.getElementById('openmarket_category_search_list_'+m); if(!s) return;
    Array.prototype.slice.call(s.options).forEach(function(o){
      if(o.value && INVALID_MARK.test(o.text||'')){
        AVOID[m]=AVOID[m]||[];
        if(AVOID[m].indexOf(o.value)<0){ AVOID[m].push(o.value); changed=true; }
      }
    });
  });
  if(changed) saveAvoid(AVOID);
}
...
function pickBest(market, opts, gender, base){
  var ok=opts.filter(function(o){ return acceptable(market, o.text, gender) && !isAvoided(market, o.val); });
...
async function processMarket(market, cls, chosenLog){
  ...
  var pool=[];
  cur.opts.forEach(function(o){ if(acceptable(market,o.text,gender) && !isAvoided(market,o.val)) pool.push({text:o.text, val:o.val}); });
  ...
  if(needSearch){
    ...
    for(var ki=0; ki<kws.length; ki++){
      ...
      opts.forEach(function(o){ if(acceptable(market,o.text,gender) && !isAvoided(market,o.val)) pool.push({text:o.text, val:o.val}); });
...
async function processFilter(state){
  ...
  learnInvalid(); // 자동매핑 전, 저장값에 붙은 "카테고리를 변경해주세요" 코드를 학습(차단 목록에 축적)
